...
function Card({ t }: { t: DBTestimonial }) {
  return (
    <article
      ...
      className="relative h-full rounded-xl p-8 transition-all duration-500 hover:border-accent-blue/30"
    >
      <div className="flex items-center gap-4">
        ...
        <div className="min-w-0">
          <div className="font-syne text-base font-semibold" style={{ color: "#ffffff" }}>
            {t.name}
          </div>
          <div
      ...
      <div className="mt-5 flex gap-0.5">
        {Array.from({ length: 5 }).map((_, i) => (
          <Star
            key={i}
            size={14}
            className={i < t.rating ? "fill-accent-blue text-accent-blue" : ""}
            style={i >= t.rating ? { color: "rgba(255,255,255,0.2)" } : {}}
          />
        ))}
      </div>

      <p
        className="mt-4 font-dm text-base italic leading-relaxed"
      ...
      <div className="container mx-auto">
        <div className="reveal mb-14 max-w-2xl">
          <div className="label-mono mb-4 flex items-center gap-3">
            ...
            <span style={{ color: "rgba(255,255,255,0.5)" }}>What Clients Say</span>
          </div>
          <h2
            className="font-syne text-4xl font-extrabold leading-tight tracking-tight md:text-5xl"
            style={{ color: "#ffffff" }}
          >
            ...
            {items.map((_, i) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all ${
                  i === idx ? "w-6 bg-accent-blue" : "w-1.5"
                }`}
                style={i !== idx ? { background: "rgba(255,255,255,0.2)" } : {}}
              />
            ))}
          </div>
        </div>
